...
void AssetManager::load_mesh(const std::filesystem::path path)
{
	...
	loaded_mesh_header.tris_count = loaded_mesh.tris.size();
	loaded_mesh_header.normals_count = loaded_mesh.normals.size();
	loaded_mesh_header.tri_idx_count = loaded_mesh.bvh->triIdx.size();
	loaded_mesh_header.bvh_node_count = loaded_mesh.bvh->bvhNodes.size();

	// Tris
	loaded_mesh_header.tris_offset = internal.consolidated_tris.size();
	...
	// Normals
	loaded_mesh_header.normals_offset = internal.consolidated_normals.size();
	...
	// Tri Idx	
	loaded_mesh_header.tri_idx_offset = internal.consolidated_tri_idxs.size();
	...
	// BVH nodes
	loaded_mesh_header.root_bvh_node_idx = internal.consolidated_nodes.size();
